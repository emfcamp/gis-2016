#!/usr/bin/env python
"""
    Original script borrowed from emfnoc

    Take the 2016 site-plan.dxf and export the following kml and csv files base on given 

    Datenklo
        DK locations based on "DK_Name" tags
        INSERT

    PowerDist
        Power grid distro based on "distro_name" tags
        POINT & INSERT

    NOC
        AP's and Switches based on "emfnet" tags
        POINT

    TODO:
    Paths
        Path and tracks base on "Paths - Euromat" & "Paths - Fire Lane" layers
        LINE

    Festoon
        Feston based on "Lighting - Festoon" layer
        LINE

    Tents
        Marquees based on "Tents" layer
...
def getLatLongForEntity(entity):
